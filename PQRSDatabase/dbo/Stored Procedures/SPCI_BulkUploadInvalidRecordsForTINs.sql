...
--Change1: Add CMS_SUbmission_Status Pavan 08/31/2021
--Change2: JIRA#1074 Pavan 12/14/2021
-- =============================================
CREATE PROCEDURE [dbo].[SPCI_BulkUploadInvalidRecordsForTINs]
...
BEGIN
	...
	if( @CategoryId = 1 )
	BEGIN
		select M.TIN,M.Measure_Name AS Measure_Number,
	...
	case when  exists(select 1 from tbl_CI_Measuredata_value md join
	  ...
	  , Case When M.IsRowEditedByUser =1 then M.ErrorMessage  ELSE 'Row Not Yet Edited' END as ErrorMessage
			...
			from tbl_CI_BulkFileUpload_History H 
			  INNER JOIN tbl_CI_BulkFileUploadCmsData M
			            ...
			            and H.FileId=M.FileId 
						 and H.IsGpro=1  
			LEFT JOIN tbl_CI_Source_UniqueKeys sc on M.TIN=sc.Tin     --Change#2                                 --Change#8: Jira#719
			...
			where m.IsRowEditedByUser=1 and (  M.IsValidata=0 or M.IsValidata is null) AND m.ErrorMessage IS NOT NULL			 
	END
	ELSE
	BEGIN			--Change2
