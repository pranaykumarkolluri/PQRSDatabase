...
-- Change#1: JIRA:914
-- =============================================
CREATE PROCEDURE [dbo].[SpBackend_TIN_IAMeasures] @CMSYear INT,
                                                    @UserName  VArchar(50)
...
AS
   ...
   -----------------TIN Cursor STARTS------------------------------
    ...
    --STEP #1: Getting Required TIN 
                     ...
                     WHERE CMSYear = @CMSYear and Is_Done=0;
                 OPEN CurTINs;
    ...
    FROM tbl_TIN_GPRO
    WHERE TIN = @strCurTIN
          AND is_GPRO = 0)--Converting  non GPRO to  GPRO
                             BEGIN
								 ...
								 [CMSYear]
                                        ...
                                        SELECT @SCOPEIDENTITY_Selection_ID,
                                              ...
                                              A.End_Date,
                                               ...
                                               GETDATE(),
                                               @CMSYear
											   from  Tbl_Backend_TIN_IA A where Tin=@strCurTIN 
...
--Change#1
                                 ...
                                 WHERE TIN = @strCurTIN
                                       AND NPI in (select NPI from @NPITable)
                                        AND CMSYear = @CMSYear);
...
--STEP#6 INSERT TIN NPI data into tbl_IA_Users
                 ...
                 FROM Tbl_Backend_TIN_IA where Tin=@strCurTIN  and CMSYear=@CMSYear and Is_Done=0								 							 				   
