...
-- =============================================
CREATE PROCEDURE [dbo].[SP_RecordEnteredPageGridDetails]
...
declare @ExamsIdsTable table(Exam_Id int, RecordsCount int);
declare @RecordsCount int=0;

insert into @ExamsIdsTable
  select Exam_Id,count(*) over() as RecordsCount from tbl_Exam where Physician_NPI=@npi and CMS_Submission_Year=@CMSYear 
...
case
 ...
 select top 1 @RecordsCount = RecordsCount from  @ExamsIdsTable
	...
	/*
			...
			select	@RecordsCount=count(*)   
												...
												tbl_Exam_Measure_Data exmes on ex.Exam_Id in(select Exam_Id from @ExamsIdsTable) and
	...
	*/
					...
					@RecordsCount  as RecordsCount
												 ...
												 from tbl_Exam ex inner join
												tbl_Exam_Measure_Data exmes on ex.Exam_Id in(select Exam_Id from @ExamsIdsTable)
												
												 and 
												 ex.Exam_Id=exmes.Exam_Id 
												and ex.Exam_Id is not null  

												join tbl_lookup_Numerator_Code N on N.Measure_ID=exmes.Measure_ID
												 and exmes.Numerator_response_value=N.Numerator_response_Value
												 and (isnull(n.Criteria,'NA') = case when exmes.Criteria is null or exmes.Criteria ='' then 'NA' else exmes.Criteria end)
												inner join tbl_Lookup_Measure Mes on exmes.Measure_ID=Mes.Measure_ID 
												left join tbl_Lookup_Data_Source d on ex.DataSource_Id=d.DataSource_Id 
												left join tbl_Lookup_Measure_Status M on exmes.Status=m.Status_ID 
