...
-- =============================================
CREATE PROCEDURE [dbo].[spReCalculatePerfomanceRateForYearandMeasureID] 
...
IF(@strMeasure_num='46')  -- Change #23 
				...
				BEGIN
					...
					EXEC spReCalculate_Measure143ForPhy @strCurNPI,@strCurTIN,@intCurActiveYear,@strMeasure_num,@blnGPRO
				END
			...
			-- Fill @@DenominatorExclusionCount
				...
				PRINT('@initPatientPopulation:['+CONVERT(varchar,ISNULL(@initPatientPopulation,0))+'],@performanceNumerator:['+CONVERT(varchar,ISNULL(@performanceNumerator,0))+']')
				PRINT('@@performanceNotMetCount:['+CONVERT(varchar,ISNULL(@performanceNotMetCount,0))+'],@@DenominatorExceptionCount:['+CONVERT(varchar,ISNULL(@DenominatorExceptionCount,0))+']')
				PRINT('@ReportingNumerator:['+CONVERT(varchar,ISNULL(@ReportingNumerator,0))+'],@ReportingDenominatorCount:['+CONVERT(varchar,ISNULL(@ReportingDenominatorCount,0))+']')		
	...
	select @decile_Val= dbo.fnYearwiseDecileLogic(@strMeasure_num,@performanceRate,@intCurActiveYear,@reportingRate,@TotalExamsCount) --change# 20
