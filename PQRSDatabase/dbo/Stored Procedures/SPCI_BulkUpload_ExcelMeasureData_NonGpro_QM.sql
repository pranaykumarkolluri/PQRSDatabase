...
-- =============================================
CREATE PROCEDURE [dbo].[SPCI_BulkUpload_ExcelMeasureData_NonGpro_QM]
	-- Add the parameters for the stored procedure here
	@Cmsyear int,
	@FileId int,
	@Createdby varchar(50),
	@CreatedDate datetime
--	 @tbl_CI_BulkFileUploadCmsData_Type_ForNonGpro tbl_CI_BulkFileUploadCmsData_Type_ForNonGpro READONLY
AS
BEGIN
...
@CreatedDate
	...
	from @tbl_CI_BulkFileUploadCmsData_Type_ForNonGpro
	*/
		IF EXISTS(SELECT 1 FROM tbl_CI_BulkFileUploadCmsData where FileId=@FileId)

	BEGIN
	--pending..
	 -- DECLARE @SUBMISSIONS INT=0;
		UPDATE tbl_CI_BulkFileUpload_History SET Status=11  where FileId=@FileId;

		/*
		WITH TEMPCTE AS
		(
			SELECT DISTINCT TIN,Npi  FROM tbl_CI_BulkFileUploadCmsData WHERE FileId=@FileId AND IsRowEditedByUser=1
	     )
	SELECT @SUBMISSIONS= COUNT(*) FROM TEMPCTE
	*/
