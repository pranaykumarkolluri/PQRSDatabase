...
-- =============================================
CREATE PROCEDURE [dbo].[spReCalculate_Measure226ForPhy90Days]
@NPI varchar(11),
@TIN varchar(10),
...
DELETE from tbl_Physician_Aggregation_For_226_90Days where CMS_Submission_Year=@intCurActiveYear   
AND Physician_NPI = @NPI 
AND   Exam_TIN = @TIN   
AND Measure_num = @strMeasure_num 
...
INSERT INTO tbl_Physician_Aggregation_For_226_90Days
...
SET @TotalExamsCount=0;
  ...
  SET @TotalExamsCount=( SELECT  COUNT(*)  FROM  tbl_Physician_Aggregation_For_226_90Days A  where  A.CMS_Submission_Year=@intCurActiveYear
                                                                                  AND A.Physician_NPI = @NPI 
																  AND  A.Exam_TIN = @TIN 
...
SET  @ReportingDenominatorCount = ISNULL(@initPatientPopulation,0) - @DenominatorExclusionCount;
  ...
  SET @performanceMetCount=(SELECT COUNT(*) from tbl_Physician_Aggregation_For_226_90Days A  where  A.CMS_Submission_Year=@intCurActiveYear   AND A.Physician_NPI = @NPI AND
   A.Exam_TIN = @TIN   AND A.Measure_Num = @strMeasure_num AND A.Performance_Met=1  AND A.Criteria=@Cur_Agg_Criteria)

   SET @performanceNotMetCount=(SELECT COUNT(*) from tbl_Physician_Aggregation_For_226_90Days A  where  A.CMS_Submission_Year=@intCurActiveYear   AND A.Physician_NPI = @NPI AND
...
SET @DenominatorExceptionCount=(SELECT COUNT(*) from tbl_Physician_Aggregation_For_226_90Days A  where  A.CMS_Submission_Year=@intCurActiveYear   AND A.Physician_NPI = @NPI AND
