...
-- =============================================
CREATE PROCEDURE [dbo].[SpBackend_GPROTIN_Measures_IA_Finalize]
...
BEGIN
	...
	IF(@isFinalize=1)
	BEGIN
     PRINT 'CODE IA:F201 - Finalization For TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) ;
	
	  --STEP #3: NISERT/UPDATE table tbl_CMS_IA_Finalization

	 IF NOT EXISTS(select * from tbl_CMS_IA_Finalization  where TIN=@strCurTIN and Finalize_Year=@CMSYear and( NPI is null or NPI =''))
	BEGIN
	  PRINT 'CODE IA:F202 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' was not finalized so we will inserting into tbl_CMS_IA_Finalization';
	...
	BEGIN
	 PRINT 'CODE IA:F203 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' was  finalized so we will updating into tbl_CMS_IA_Finalization for Finalization'
	...
	BEGIN
	 ...
	 IF NOT EXISTS(select * from tbl_CMS_IA_Finalization where TIN=@strCurTIN and Finalize_Year=@CMSYear and( NPI is null or NPI =''))
	BEGIN
	   PRINT 'CODE:F 207 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' was not UNDO finalized so we will inserting into tbl_CMS_IA_Finalization';
	...
	BEGIN
  PRINT 'CODE IA:F 208 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' was UNDO finalized so we will updating into tbl_CMS_IA_Finalization';
   ...
   SET 
    ...
    --STEP #4: NISERT/UPDATE table tbl_TINConvertion_Lock
	  ...
	  if not exists(select Id from tbl_TINConvertion_Lock  where TIN=@strCurTIN and CMSYear=@CMSYear and isGpro=1 and Npi is null)
		begin 
		 PRINT 'CODE IA:F209 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' value insert into tbl_TINConvertion_Lock for Locking'
		insert into
		 tbl_TINConvertion_Lock
		(TIN,
		isGpro,
