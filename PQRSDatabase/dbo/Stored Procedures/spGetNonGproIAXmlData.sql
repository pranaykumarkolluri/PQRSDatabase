-- =============================================
-- Author:		Raju Gaddam
-- Create date: March 14,2018
-- Description:	Get IA NonGpro Activities for Generate IA XML 2017
--change 1#: hari j on 31/10/2018
--chnage#1 -- no need to use   NRDR..[PHYSICIAN_TIN_VW]
--Change#2 Sumanth
--Change#2:Jira-784
-- =============================================
CREATE PROCEDURE [dbo].[spGetNonGproIAXmlData]

@CMSYear int
AS
BEGIN

--change#1
--Declare @PhysicinaTins as Table(NPI Varchar(10),
--TIN Varchar(9))
--INSERT into @PhysicinaTins
--select DISTINCT  NPI,TIN from NRDR..[PHYSICIAN_TIN_VW]

select distinct S.SelectedActivity,I.TIN,I.NPI,I.CMSYear 
--from tbl_Physician_TIN P
--from @PhysicinaTins P 
--join tbl_IA_Users I on P.TIN=I.TIN

from tbl_IA_Users I  with (nolock)
join tbl_IA_User_Selected S  with (nolock) on S.SelectedID =I.SelectedID 
where I.CMSYear=@CMSYear 
and I.TIN not in(select TIN from tbl_TIN_GPRO where is_GPRO=1)
and ((@CMSYear>=2020 and I.NPI is not null ) or @CMSYear<2020 )     --Change#2
--group by S.SelectedActivity,I.SelectedID,I.TIN,I.NPI,I.CMSYear

END



