...
--Change#2: Jira#970 Pavan 06/08/2021 
--Change#3: Jira#1074 Pavan 12/14/2021 
-- =============================================
CREATE PROCEDURE [dbo].[SPIA_FinalizeDetails_Physician]	
...
BEGIN		
		...
		declare @is_enrolled bit;    --Change#2
		...
		--END
						   ...
						   BEGIN
							 ...
							 CASE WHEN (I.FinalizeEmail IS NULL OR I.FinalizeEmail='') THEN ''
							  ...
							  Convert(varchar(250),I.FinalizeAgreeTime) as finalizeAgreeDate,
								...
								@is_enrolled as is_enrolled	--Change#2
									from @UserRelatedTINSwithGpro f left join tbl_CMS_IA_Finalization I
			...
			Select @SubmittoCMS_Status=IsSubmittoCMS from tbl_Lookup_Active_Submission_Year where Submission_Year=@CMSYear and IsActive=1    --Change#1:			   	
						    select distinct 														
								...
								dbo.fnCI_GetLastSubmittedUser(f.TIN,@Npi,@CMSYear,@Category_Id) as LastSubmittedBy,		--Change#3
								@CMSYear as CMSYear,
