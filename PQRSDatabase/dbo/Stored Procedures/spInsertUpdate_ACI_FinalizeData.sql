...
-- =============================================
CREATE PROCEDURE [dbo].[spInsertUpdate_ACI_FinalizeData]
  ...
  --TIN level insert/update data
	...
	begin
	  print 'enter gpro level';
		if not exists (select  Fid  from tbl_CMS_ACI_Finalization  where TIN=@Tin and isGpro=1 and Finalize_Year=@CMSYear and NPI is null)
		begin 
		--inserting 
         print 'enter tin leve gpro';
		insert into 
		tbl_CMS_ACI_Finalization(TIN,isGpro,
		...
		begin
				if(@isFinalize=0)
				begin
							update tbl_CMS_ACI_Finalization 
					set
		...
		(TIN,isGpro,CMSYear,isACIFinalize,isLock,CreatedBy,CreatedDate)
		values
	...
	begin
		...
		if not exists (select Fid from tbl_CMS_ACI_Finalization  where TIN=@Tin and isGpro=@isGpro and Finalize_Year=@CMSYear and NPI=@Npi)
		begin 
		--inserting 
		insert into 
		tbl_CMS_ACI_Finalization(TIN,NPI,isGpro,
		isFinalize,FinalizeEmail,Finalize_Year
		,FinalizeAgreeTime,CreatedBy,CreatedDate)
		values
		...
		begin 
		insert into tbl_TINConvertion_Lock
		(TIN,NPI,isGpro,CMSYear,isACIFinalize,isLock,CreatedBy,CreatedDate)
		values
