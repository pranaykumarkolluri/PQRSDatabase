...
-- Change #8:  RAJU G
-- Change Desc: For JIRA#806
-- =============================================
CREATE PROCEDURE [dbo].[spValidateImportExam_ByTwoActiveYears] 
  ...
  --print('spValidateImportExam validate-2 started'+CONVERT( VARCHAR(24), GETDATE(), 113));
	...
	-- set @iParentNode = @ParentNode  + '\Facility Id:' + ISNULL(@Import_Facility_ID,'<missing>')  + '\Unique Exam Id:' + ISNULL(@Import_Exam_Unique_ID,'<missing>') 
                 ...
                 --   OR ( ISNULL(@Import_Exam_DateTime, '') = '' ) 
                     ...
                     --    END;
					 ELSE
					  IF NOT EXISTS(SELECT 1 FROM tbl_Lookup_Active_Submission_Year WHERE IsActive=1 AND Submission_Year = YEAR(@Import_Exam_DateTime) )
					  BEGIN
					          SET @message = @message+'P3013:Active CMS submission years not found with examdate['+@Import_Exam_DateTime+']'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
					  END
