...
--Change#7: Fetching TotalExamCount from aggregation table instead of tbl_exam and tbl_exam_measuredata
--Change#7:By Raju G date:feb-13-2019
--Change#8:Sumanth J: 03 mar 2019
--Change#8:Jira #672
--Change#9:JIRA#696: Display priority symbol for a measure in CMS submission and Performance report page
--Change#10:Jira#719
--Change#11: Jira#750
-- =============================================
CREATE PROCEDURE [dbo].[spNPIMeasureRelatedmeasureDataForFullYear] 
...
select @IsGpro=is_GPRO from tbl_TIN_GPRO where TIN=@TIN
				  ...
				  case when G.HundredPercentSubmit=1 then G.HundredPercentSubmit
					...
					else CONVERT(bit,0) end as HundredPercentSubmit,   --this is screenedForUse/c1 exam count for 226
				...
				0 as TotalCasesReviewed_C3,
					...
					--													    ELSE 0 END as TotalExamsCount_C3
						0 as TotalExamsCount_C3,
							...
							else '' end as PR_CMeasure,        --Change#8
							LP.[Name] as PriorityName,          --Change#9
							...
							ELSE CONVERT(bit,0) end as ISSubmittedtoCMS,                  --Change#10: Jira#719
							LM.Is_eCQM              --Change#11
		from tbl_Physician_Aggregation_Year A 
					...
					INNER JOIN tbl_Lookup_Measure LM ON LM.CMSYear=@CMSYear 
													...
													AND A.Is_90Days=0 
					LEFT JOIN tbl_Lookup_Measure_Priority LP on LM.Priority_ID=LP.Priority_ID      --Change#9
				    ...
				    LEFT JOIN tbl_CI_Source_UniqueKeys sc on A.Exam_TIN=sc.Tin                            --Change#10: Jira#719
              ...
              --      LEFT JOIN tbl_CI_Measuredata_value md on sc.Key_Id=md.KeyId                                 --Change#10: Jira#719
				...
				order by LM.DisplayOrder
					--and (ISNULL(a.Stratum_Id,0)=0 OR a.Stratum_Id=1)--Change#2
