...
-- =============================================
CREATE PROCEDURE [dbo].[spTinLevelACIFinalizationValidation]
...
--Step #1 tin level attestation validation
		...
		select case
						...
						when (select COUNT(*) 
		from  tbl_ACI_TIN_LevelAttestation where GPROTIN =@TIN 
		...
		when (select count(*) from tbl_ACI_Users where TIN=@TIN
