...
-- Description:	JIRA#
-- =============================================
CREATE PROCEDURE spReCalculateTINperformanceRateForYear_NewSP
	@intYear int = 0, 
	@strTIN varchar(11) = ''
AS
BEGIN
...
DECLARE @tbl_TIN_Aggr AS TABLE(
	...
	Delete from tbl_TIN_Aggregation_Year_ForNewSP where CMS_Submission_Year = @intYear
	...
	DECLARE @strCurTIN VARCHAR(9);
	 DECLARE CurTINS CURSOR FOR 
				...
				PRExamMeasure_VW WITH ( NOEXPAND )  where  CMS_Submission_Year = @intYear 
				and  Exam_tin = CASE  @strTIN WHEN '' THEN Exam_tin ELSE @strTIN END
...
SELECT E.Exam_TIN, E.Measure_ID, E.Measure_num,SUM(E.MeasureCount) as MeasuresCount FROM PRExamMeasure_VW E WITH ( NOEXPAND ) 
        WHERE e.CMS_Submission_Year = @intYear    
		and  Exam_tin = CASE  @strTIN WHEN '' THEN Exam_tin ELSE @strTIN END
	        AND E.Measure_num NOT IN ('226','143','46')   
...
@intYear	as CMS_Submission_Year,
	...
	0 as Stratum_Id,
	( SELECT count( distinct E.Physician_NPI)  FROM PRExamMeasure_VW E WITH ( NOEXPAND ) 
        WHERE e.CMS_Submission_Year = @intYear     
...
;WITH PHYTOTALCASES AS(
  select SUM(isnull (case S.HundredPercentSubmit
										...
										from PRExamMeasure_VW A WITH ( NOEXPAND ) INNER JOIN   tbl_Physician_Selected_Measures S ON  
...
@intYear	as CMS_Submission_Year,
 ...
 --       WHERE e.CMS_Submission_Year = @intYear     
  ...
  FROM tbl_Non_PQRS_TIN_Aggregation_Year t where CMS_Submission_Year=@intYear
  -------------------
...
UPDATE A SET  Reporting_Rate= CASE WHEN Reporting_Denominator>0 THEN ((CAST(Reporting_Numerator  as float)/Reporting_Denominator) *100) ELSE NULL END
	...
	INSERT INTO [dbo].[tbl_TIN_Aggregation_Year_ForNewSP]
           ([CMS_Submission_Year]
           ,[CMS_Submission_Date]
           ,[Exam_TIN]
           ,[Measure_Num]
...
-----------Performance Calculation For MIPS Measures '226','46','143'   ------STARTS
	 ...
	 DECLARE CurStratumMes CURSOR FOR 
      ...
      SELECT e.Exam_TIN,E.Measure_ID, E.Measure_num FROM PRExamMeasure_VW E WITH ( NOEXPAND ) 
        WHERE e.CMS_Submission_Year = @intYear     
	      AND E.Measure_num IN ('226','143','46')  
                OPEN CurStratumMes
                ...
                WHILE @@FETCH_STATUS = 0 
                BEGIN	
			 -- SELECT TOP 1 @blnGPRO from tbl_TIN_GPRO WHERE TIN=@strCurTIN
  ...
  --------------
  	update tbl_TIN_Aggregation_Year_ForNewSP
				set
