...
-- =============================================
-- Author:		Harikrishna Jubburu & Raju
...
--Change #1 : JIRA#1012 By Sai, 16th Aug,2021 -- Adding performance Calculation SP's
-- =============================================
CREATE PROCEDURE [dbo].[SPCI_BulkUpload_MeasureDataUpdateForFacility]
	@IsGPORO bit ,
	@FileId int
...
BEGIN
  ...
  --Step#: Get editable rows from particular file
   ...
   begin
   print 'file id status update 21'+Convert(varchar(50),@FileId);
   ...
   SET  SelectedForCms_new =CASE 
						...
						WHEN	  (UPPER(SelectedForCms_new) ='NO') OR
						 ...
						 WHEN ISNULL(LTRIM(RTRIM(SelectedForCms_new)),'')=''  
						 THEN NULL 
   ...
   HundredPercentSubmit_new =CASE 
					    ...
					    WHEN ISNULL(LTRIM(RTRIM(HundredPercentSubmit_new)),'')=''  
						 THEN NULL 
						  
						   ELSE HundredPercentSubmit_new			
				   END 
   ,
   EndtoEndReporting_new =Case WHEN  (UPPER(EndtoEndReporting_new)='YES') OR
							 ...
							 WHEN ISNULL(LTRIM(RTRIM(EndtoEndReporting_new)),'')=''  
						    THEN NULL 
							ELSE EndtoEndReporting_new	 
								  END
   --,
...
declare @tbl_FacilityTIN_GPRO table(

Tin varchar(9),
IS_GPRO bit
);
INSERT INTO @tbl_FacilityTIN_GPRO
exec [NRDR]..[sp_getFacilityTIN_GPRO] @FacilityUserName
  ...
  UPDATE [tbl_CI_BulkFileUploadCmsData]
   ...
   SET  ErrorMessage = 'TIN is not authorized for this User ['+@FacilityUserName+']'  ,
        IsValidata=0
   WHERE FileId=@FileId 
   and [IsRowEditedByUser]=1 
   and IsValidata=1
   and TIN not in(select DISTINCT Tin from @tbl_FacilityTIN_GPRO )
...
16	PostedPartially	
  */
  ...
  /*
     SELECT
P.SelectedForSubmission AS DS ,
P.Measure_num AS dm,
CASE WHEN  B.SelectedForCms_new ='1'  THEN 1 
					...
					ELSE 	p.SelectedForSubmission END AS CS,

b.SelectedForCms_new AS ES,
B.Measure_Name AS eM
...
tbl_GPRO_TIN_Selected_Measures P
				 ...
				 WHERE 
						...
						and p.TIN in (select distinct tin from tbl_CI_BulkFileUploadCmsData  where
						  fileId=@FileId
						  and
...
UPDATE [dbo].[tbl_CI_BulkFileUpload_History]
   SET
	 ...
	 -- ,CmsStatus= CASE WHEN (SELECT COUNT(*) from tbl_CI_BulkFileUploadCmsData where FileId=@FileId and IsValidata=1)=0 THEN 12 ELSE CmsStatus END
 ...
 WHERE  FileId=@FileId AND Status IN (12,14,13) and Status !=21
