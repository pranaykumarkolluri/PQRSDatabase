...
-- Change #23 JIRA#609
-- =============================================
CREATE PROCEDURE [dbo].[spReCalculate_StratumCalbyAgeforTIN90Days] 
	   ...
	   on s.Measure_Num=m.Measure_num where m.Measure_ID=@intCurMeasureId and m.CMSYear=@intCurActiveYear
		...
		print('measure 46 related stratum code:inside  CURSUR')
				...
				select @decile_Val= dbo.fnYearwiseDecileLogic(@strMeasure_num,@performanceRate,@intCurActiveYear,@reportingRate,@TotalExamsCount) 
				 ...
				 --print 'first inserted line 451 totalPhysiansSubmittedCount'+Convert(varchar,@totalPhysiansSubmittedCount); 
				  --print 'first inserted line 452 [TotalExamsCount]-@strCurTIN'+Convert(varchar,@TotalExamsCount)+'tin'+@strCurTIN+'Measure_Num'+@strMeasure_num; 
