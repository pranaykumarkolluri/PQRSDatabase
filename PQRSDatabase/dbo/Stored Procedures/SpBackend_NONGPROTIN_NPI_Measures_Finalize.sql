...
-- =============================================
CREATE PROCEDURE [dbo].[SpBackend_NONGPROTIN_NPI_Measures_Finalize]
...
BEGIN
	...
	IF(@isFinalize=1)
	BEGIN
     PRINT 'CODE:FTN 201 - Finalization For TIN: ' + CAST(@strCurTIN AS VARCHAR(10))+' NPI: ' + CAST(@strCurNPI AS VARCHAR(10));
	
	  --STEP #3: INSERT/UPDATE table tbl_CMS_Finalization

	 IF NOT EXISTS(select * from tbl_CMS_Finalization  where TIN=@strCurTIN and Finalize_Year=@CMSYear and NPI =@strCurNPI)
	BEGIN
	  PRINT 'CODE:FTN 202 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' NPI: ' + CAST(@strCurNPI AS VARCHAR(10)) +' was not finalized so we will inserting into tbl_CMS_Finalization';
	...
	BEGIN
	 PRINT 'CODE:FTN 203 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' NPI: ' + CAST(@strCurNPI AS VARCHAR(10)) +' was  finalized so we will updating into tbl_CMS_Finalization for Finalization'
	...
	--STEP #4: INSERT/UPDATE table tbl_TINConvertion_Lock
	  ...
	  if not exists(select Id from tbl_TINConvertion_Lock  where TIN=@strCurTIN and CMSYear=@CMSYear and isGpro=0 and Npi=@strCurNPI)
		begin 
		 PRINT 'CODE:FTN 204 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' NPI: ' + CAST(@strCurNPI AS VARCHAR(10)) +' value insert into tbl_TINConvertion_Lock for Locking'
		insert into
		...
		begin
		 ...
		 PRINT 'CODE:FTN 205 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' NPI: ' + CAST(@strCurNPI AS VARCHAR(10)) +' value updating into tbl_TINConvertion_Lock for Locking'
	...
	BEGIN
	 PRINT 'CODE:FTN 206 - UNDO Finalization For TIN: ' + CAST(@strCurTIN AS VARCHAR(10))+' NPI: ' + CAST(@strCurNPI AS VARCHAR(10)) ;

	   --STEP #3: INSERT/UPDATE table tbl_CMS_Finalization
	 IF NOT EXISTS(select * from tbl_CMS_Finalization where TIN=@strCurTIN and Finalize_Year=@CMSYear and Npi=@strCurNPI)
	BEGIN
	 
	   PRINT 'CODE:FTN 207 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' NPI: ' + CAST(@strCurNPI AS VARCHAR(10)) +' was not UNDO finalized so we will inserting into tbl_CMS_Finalization';
	...
	BEGIN
 PRINT 'CODE:FTN 208 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' NPI: ' + CAST(@strCurNPI AS VARCHAR(10)) +' was UNDO finalized so we will updating into tbl_CMS_Finalization for Finalization'
   ...
   SET 
    ...
    --STEP #4: INSERT/UPDATE table tbl_TINConvertion_Lock
	  ...
	  if not exists(select Id from tbl_TINConvertion_Lock  where TIN=@strCurTIN and CMSYear=@CMSYear and isGpro=0 and Npi=@strCurNPI)
		begin 
		 PRINT 'CODE:FTN 209 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' NPI: ' + CAST(@strCurNPI AS VARCHAR(10)) +' value insert into tbl_TINConvertion_Lock for Locking'
		insert into
		 tbl_TINConvertion_Lock
		(TIN,
		NPI,
		...
		begin
		 ...
		 PRINT 'CODE:FTN 210 -TIN: ' + CAST(@strCurTIN AS VARCHAR(10)) +' NPI: ' + CAST(@strCurNPI AS VARCHAR(10)) +' value updating into tbl_TINConvertion_Lock for Locking'
		update tbl_TINConvertion_Lock
		 set 	
		 isQMFinalize=0,
		 LastModifiedBy=@UserID,
