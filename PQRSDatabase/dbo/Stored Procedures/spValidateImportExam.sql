...
-- Change #7:  Hari J
-- Change Date: 13-Sept-19
-- Change Desc: For JIRA#735
-- Change #8 Pavan A
-- Change Date: 12-Jan-2022
-- Change Desc: For JIRA#1096
-- =============================================
CREATE PROCEDURE [dbo].[spValidateImportExam] 
...
AS
   ...
   --print('spValidateImportExam validate-1 started'+CONVERT( VARCHAR(24), GETDATE(), 113));	

             DECLARE @Import_examID INT, @Import_Physician_Group_TIN VARCHAR(50), @Import_Exam_Unique_ID VARCHAR(500), @Import_Exam_DateTime VARCHAR(50), @Import_Physician_NPI VARCHAR(50), @Import_First_Name VARCHAR(50), @Import_Last_Name VARCHAR(50), @Import_Patient_ID VARCHAR(50), @Import_Patient_Age VARCHAR(50), @Import_Patient_Gender VARCHAR(50), @Import_Patient_Medicare_Beneficiary VARCHAR(50), @Import_Patient_Medicare_Advantage VARCHAR(50), @Import_Num_of_Measures_Included VARCHAR(50);
			 DECLARE @Decrypt_Patient_ID VARCHAR(50)
...
--
	  ...
	  -- Change #5: 
	    ...
	    -- Change #6: 
             DECLARE @intInCorrect_Measure_DataWith_ExclusionCount INT;
		
--	DECLARE @x as varchar(50)

             DECLARE Cursor_Imp_Exam CURSOR
             FOR SELECT Import_examID,
             ...
             FETCH NEXT FROM Cursor_Imp_Exam INTO @Import_examID, @Import_Physician_Group_TIN, @Import_Exam_Unique_ID, @Import_Exam_DateTime, @Import_Physician_NPI, @Import_First_Name, @Import_Last_Name, @Import_Patient_ID,@Decrypt_Patient_ID, @Import_Patient_Age, @Import_Patient_Gender, @Import_Patient_Medicare_Beneficiary, @Import_Patient_Medicare_Advantage, @Import_Num_of_Measures_Included;
             WHILE @@FETCH_STATUS = 0
                 BEGIN
                     SET @Import_Exam_Unique_ID = CASE
                                                      WHEN @Import_Exam_Unique_ID IS NULL
                                                      THEN ''
                                                      ELSE @Import_Exam_Unique_ID
                                                  END;
                     SET @Import_Exam_DateTime = CASE
                                                     WHEN @Import_Exam_DateTime = ''
                                                     THEN NULL
                                                     ELSE @Import_Exam_DateTime
                                                 END;
                     SET @Import_Physician_NPI = CASE
                                                     WHEN @Import_Physician_NPI IS NULL
                                                     THEN ''
                                                     ELSE @Import_Physician_NPI
                                                 END;
                     SET @Import_First_Name = CASE
                                                  WHEN @Import_First_Name IS NULL
                                                  THEN ''
                                                  ELSE @Import_First_Name
                                              END;
                     SET @Import_Last_Name = CASE
                                                 WHEN @Import_Last_Name IS NULL
                                                 THEN ''
                                                 ELSE @Import_Last_Name
                                             END;
                     SET @Import_Patient_ID = CASE
                                                  WHEN @Import_Patient_ID IS NULL
                                                  THEN ''
                                                  ELSE @Import_Patient_ID
                                              END;
					SET @Decrypt_Patient_ID = CASE 
												WHEN @Decrypt_Patient_ID IS NULL
												Then ''
												ELSE @Decrypt_Patient_ID
												END;
		
  --print('spValidateImportExam validate-2 started'+CONVERT( VARCHAR(24), GETDATE(), 113));
                     SET @message = '';
                     SET @No_of_Errors = 0;
                     SET @TransactionID = CASE
	...
	-- set @iParentNode = @ParentNode  + '\Facility Id:' + ISNULL(@Import_Facility_ID,'<missing>')  + '\Unique Exam Id:' + ISNULL(@Import_Exam_Unique_ID,'<missing>') 
                     SET @iParentNode = '';
                     IF(@Import_Exam_Unique_ID = '')
                         BEGIN
                             SET @message = @message+'P3001:Missing Exam_Unique_ID'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                     IF(@Import_Exam_Unique_ID <> '')
                         BEGIN
                             SELECT @message = @message+'P3004: Exam_Unique_ID must be unique. Exam_Unique_ID ['+@Import_Exam_Unique_ID+'] is submitted multiple times in Transaction ID ['+@TransactionID+'].'+CHAR(10),
                                    @No_of_Errors = @No_of_Errors + 1
                             FROM tbl_Import_Exam
                             WHERE Import_examID = @Import_examID
                                   AND ISNULL(Import_Exam_Unique_ID, '') = @Import_Exam_Unique_ID
                             GROUP BY Import_examID,
                                      Import_Exam_Unique_ID
                             HAVING COUNT(Import_Exam_Unique_ID) > 1;
                         END;
					-------- Change #8
					IF(@Import_Exam_Unique_ID LIKE '%[^a-zA-Z0-9]%')
						BEGIN
							 SET @message = @message+'P3002:Exam Id cannot contain special Characters'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
						END
					--------
                 ...
                 --   OR ( ISNULL(@Import_Exam_DateTime, '') = '' ) 
                         BEGIN
                             SET @message = @message+'P3011:Missing Exam_Date'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                         ELSE
                     IF ISDATE(@Import_Exam_DateTime) = 0
                         BEGIN
                             SET @message = @message+'P3012:Exam_Date ('+@Import_Exam_DateTime+')is not a valid date time in format mm/dd/yyyy'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                         ELSE
                     IF(ISDATE(@Import_Exam_DateTime) = 1
                        AND (CONVERT(DATETIME, @Import_Exam_DateTime, 101) > GETDATE()))
                         BEGIN
                             SET @message = @message+'P3013:Exam_Date ('+@Import_Exam_DateTime+')is future dated. Tested format mm/dd/yyyy'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                     IF(@Import_First_Name = '')
                         BEGIN
                             SET @message = @message+'P3021:Missing Physician_First_Name'+CHAR(10)+@iParentNode+']';
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                     IF(@Import_Last_Name = '')
                         BEGIN
                             SET @message = @message+'P3031:Missing Physician_Last_Name'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                     IF(@Import_Physician_NPI = '')
                         BEGIN
                             SET @message = @message+'P3041:Missing Physician_NPI in submitted data'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                         ELSE
                     IF NOT EXISTS
    ...
    WHERE LTRIM(RTRIM(NPI)) = LTRIM(RTRIM(@Import_Physician_NPI))
				    ...
				    ---TODO:
                         ...
                         -- Change #4
                             ...
                             EXEC dbo.SPCheck_NPI_Exists
                                  @Import_Physician_NPI,
                                  @StatusCode = @StatusCode OUTPUT;
                             IF(@StatusCode <> 1)--1=new record inserted in tbl_users table
                                 BEGIN
                                     SET @message = @message+'P3042:Entered Physician_NPI('+@Import_Physician_NPI+') has not registered for PQRS. Physician is required to register in order to participate.'+CHAR(10);
                                     SET @No_of_Errors = @No_of_Errors + 1;
                                 END;
                         END;
                         ELSE
    ...
    WHERE LTRIM(RTRIM(NPI)) = LTRIM(RTRIM(@Import_Physician_NPI))
          AND (Attested = 0
               OR Attested IS NULL)
                         ...
                         BEGIN
                             SET @message = @message+'P3043:Entered Physician_NPI('+@Import_Physician_NPI+') has registered for PQRS. However Physician is yet to accept the terms and conditions in order to participate.'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                         ELSE
                         BEGIN
...
--set @x  = ''
					 --   select @x = @FacilityID;
  --print('spValidateImportExam validate-3 started'+CONVERT( VARCHAR(24), GETDATE(), 113));
						--old nrdr10x code
...
(
    SELECT  1 
    FROM [NRDR]..physician p WITH (nolock)
...
(
    SELECT physician_id
    FROM [NRDR]..physician_to_facility_to_registry
    WHERE facility_id = @FacilityID
))
          AND p.NPI = @Import_Physician_npi
)
                                 BEGIN
  --print('spValidateImportExam validate-3 if inside'+CONVERT( VARCHAR(24), GETDATE(), 113));
                                     SET @message = @message+'P3044:PhysicianNPI( '+@Import_Physician_NPI+' ) you have submitted is not under facilityID( '+@FacilityID+' )'++CHAR(10);
                                     SET @No_of_Errors = @No_of_Errors + 1;
                                 END;

  --print('spValidateImportExam validate-4 inside'+CONVERT( VARCHAR(24), GETDATE(), 113));
/*
...
--chnaged by HARI j on APRIL/13/2018
  ...
  --              IF EXISTS ( SELECT 1 FROM [NRDR]..REGISTRY_TIN RT
		...
		--print('spValidateImportExam validate-5 inside'+CONVERT( VARCHAR(24), GETDATE(), 113));
                     IF EXISTS
(
    SELECT 1
    FROM NRDR..PHYSICIAN_TO_FACILITY_TO_REGISTRY f
         INNER JOIN NRDR..REGISTRY_TIN RT ON f.FACILITY_ID = RT.FACILITY_ID
         JOIN NRDR..PHYSICIAN p ON p.ID = f.PHYSICIAN_ID
    WHERE(P.NPI = @Import_Physician_NPI)
         AND (RT.NUMBER = @Import_Physician_Group_TIN)
)
                         BEGIN
                             SET @Count = 0;
                         END;
                     IF(LTRIM(RTRIM(ISNULL(@Import_Physician_Group_TIN, '')))) = ''
                         BEGIN
                             SET @message = @message+'P3051:Missing Physician_Group_TIN in submitted data.'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                         ELSE
                     IF dbo.IsInteger(LTRIM(RTRIM(ISNULL(@Import_Physician_Group_TIN, '')))) = 0
                         BEGIN
                             SET @message = @message+'P3053:Invalid Physician_Group_TIN ['+LTRIM(RTRIM(ISNULL(@Import_Physician_Group_TIN, '')))+'] must be an integer of 9 digits.'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                         ELSE
                     IF LEN(LTRIM(RTRIM(ISNULL(@Import_Physician_Group_TIN, '')))) <> 9
                         BEGIN
                             SET @message = @message+'P3052:Entered Physician_Group_TIN ['+LTRIM(RTRIM(ISNULL(@Import_Physician_Group_TIN, '')))+'] must be of 9 digits.'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                         ELSE
                     IF(@Count = 1)
                         BEGIN
                             SET @message = @message+'P3054:Entered Physician_Group_TIN ['+LTRIM(RTRIM(@Import_Physician_Group_TIN))+'] and Physician_NPI ['+LTRIM(RTRIM(@Import_Physician_NPI))+'] mapping not found. '+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                     IF(@Import_Patient_ID IS NULL)
                       OR (ISNULL(@Import_Patient_ID, '') = '')
                         BEGIN
                             SET @message = @message+'P3061:Missing Patient_ID'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
					--------- Change #8
					IF(@Decrypt_Patient_ID LIKE '%[^a-zA-Z0-9]%')
						BEGIN
							 SET @message = @message+'P3062:Patient_Id cannot contain special Characters'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
						END;
					----------------
                     IF(@Import_Patient_Age IS NULL)
                       OR (ISNULL(@Import_Patient_Age, '') = '')
                         BEGIN
                             SET @message = @message+'P3071:Missing Patient_Age'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                         ELSE
                     IF dbo.IsInteger(ISNULL(@Import_Patient_Age, '')) = 0
                         BEGIN
                             SET @message = @message+'P3072:Entered Patient_Age ['+LTRIM(RTRIM(ISNULL(@Import_Patient_Age, '')))+'] not a valid 1 to 3 digit integer.'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                         ELSE
                     ...
                     AND ((CONVERT(INTEGER, @Import_Patient_Age) < 1
                         ...
                         BEGIN
                             SET @message = @message+'P3073:Entered Patient_Age ['+LTRIM(RTRIM(ISNULL(@Import_Patient_Age, '')))+'] is out of range. Valid range is 1 to 130.'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;

--print('spValidateImportExam validate-6 inside'+CONVERT( VARCHAR(24), GETDATE(), 113));
	...
	--	End
                     ...
                     IF(@Import_Patient_Gender IS NULL)
                       OR (ISNULL(@Import_Patient_Gender, '') = '')
                         BEGIN
                             SET @message = @message+'P3081:Missing Patient_Gender'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                         ELSE
                     IF(LOWER(ISNULL(@Import_Patient_Gender, '')) <> 'm'
                        ...
                        AND LOWER(ISNULL(@Import_Patient_Gender, '')) <> 'o')
                         BEGIN
                             SET @message = @message+'P3082:Entered Patient_Gender ['+LTRIM(RTRIM(ISNULL(@Import_Patient_Gender, '')))+'] must be "M" or "F" or "U" or "O".'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
	
		
    --print('spValidateImportExam validate-7 inside'+CONVERT( VARCHAR(24), GETDATE(), 113));
	...
	--Else
                     IF(UPPER(ISNULL(@Import_Patient_Medicare_Beneficiary, '')) <> 'Y'
                        ...
                        AND UPPER(ISNULL(@Import_Patient_Medicare_Beneficiary, '')) <> 'NA')
                         BEGIN
                             SET @message = @message+'P3102:Entered Patient_Medicare_Beneficiary ['+LTRIM(RTRIM(ISNULL(@Import_Patient_Medicare_Beneficiary, '')))+'] must be "Y" for "Yes" or "N" for "No" or "NA" for "Not Available".'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
   ...
   --Else
                     ...
                     IF(UPPER(ISNULL(@Import_Patient_Medicare_Advantage, '')) <> 'Y'
                        ...
                        AND UPPER(ISNULL(@Import_Patient_Medicare_Advantage, '')) <> 'NA')
                         BEGIN
                             SET @message = @message+'P3112:Entered Patient_Medicare_Advantage ['+LTRIM(RTRIM(ISNULL(@Import_Patient_Medicare_Advantage, '')))+'] must be "Y" for "Yes" or "N" for "No" or "NA" for "Not Available".'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
                         ELSE -- Change #7
					 IF(UPPER(ISNULL(@Import_Patient_Medicare_Beneficiary, '')) = 'Y'
					 AND (@Import_Patient_Medicare_Advantage ='NA')
					 )
                         BEGIN
                            
                                     SET @message = @message+'P3113: Required "Patient medicare advantage" if  response to “Patient medicare beneficiary” is ‘Y”.'+CHAR(10);
                                     SET @No_of_Errors = @No_of_Errors + 1;
                                 
                         END;
                     --IF(UPPER(ISNULL(@Import_Patient_Medicare_Beneficiary, '')) = 'N')
                     --    BEGIN
                     --        IF(UPPER(ISNULL(@Import_Patient_Medicare_Advantage, '')) <> 'N'
                     --           AND UPPER(ISNULL(@Import_Patient_Medicare_Advantage, '')) <> 'NA')
                     --            BEGIN
                     --                SET @message = @message+'P3113: Invalid Patient_Medicare_Advantage.'+CHAR(10);
                     --                SET @No_of_Errors = @No_of_Errors + 1;
                     --            END;
                     --    END;
                     --    ELSE
                     --IF(UPPER(ISNULL(@Import_Patient_Medicare_Beneficiary, '')) = 'NA')
                     --    BEGIN
                     --        IF(UPPER(ISNULL(@Import_Patient_Medicare_Advantage, '')) <> 'NA')
                     --            BEGIN
                     --                SET @message = @message+'P3114: Invalid Patient_Medicare_Advantage.'+CHAR(10);
                     --                SET @No_of_Errors = @No_of_Errors + 1;
                     --            END;
                     --    END;
                     IF(@Import_Num_of_Measures_Included IS NULL)
                       OR (ISNULL(@Import_Num_of_Measures_Included, '') = '')
                         BEGIN
                             SET @message = @message+'P3121:Missing Num_of_Measures_Included'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
...
(
    ...
    WHERE Import_ExamID = @Import_examID
                         ...
                         BEGIN
                             SET @message = @message+'P3131:Missing Measure_Data'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                             ...
                             IF dbo.IsInteger(@Import_Num_of_Measures_Included) = 1
                                 BEGIN
                                     IF(@intMeasuresCount <> CONVERT(INT, @Import_Num_of_Measures_Included))
                                         BEGIN
                                             SET @message = @message+'P3123: Data in Num_of_Measures_Included ('+@Import_Num_of_Measures_Included+') does not match with Measures received ('+CONVERT(VARCHAR(10), @intMeasuresCount)+')'+CHAR(10);
                                             SET @No_of_Errors = @No_of_Errors + 1;
                                         END;
                                 END;
                                 ELSE
                                 BEGIN
                                     IF(ISNULL(@Import_Num_of_Measures_Included, '') <> '')
                                         BEGIN
                                             SET @message = @message+'P3122: Data in Num_of_Measures_Included  ('+@Import_Num_of_Measures_Included+') is not an integer. '+CHAR(10);
                                             SET @No_of_Errors = @No_of_Errors + 1;
                                         END;
                                 END;
                         END;
		--print('spValidateImportExam validate-8 inside'+CONVERT( VARCHAR(24), GETDATE(), 113));

                     SET @strDuplicateMeasuresinExam = '';
                     SELECT @strDuplicateMeasuresinExam = @strDuplicateMeasuresinExam+CASE @strDuplicateMeasuresinExam
                                                                                          WHEN ''
                                                                                          THEN ''
                                                                                          ELSE ','
                     ...
                     IF(@strDuplicateMeasuresinExam <> '')
                         BEGIN
                             SET @message = @message+'P3502: Following Measures ['+@strDuplicateMeasuresinExam+'] have been submitted multiple times for Exam Unique ID ['+ISNULL(@Import_Exam_Unique_ID, '')+']'+CHAR(10);
                             SET @No_of_Errors = @No_of_Errors + 1;
                         END;
		
			--print('spValidateImportExam validate-9 inside'+CONVERT( VARCHAR(24), GETDATE(), 113));
...
-- end of peer level validations		
                     ...
                     SET @intInCorrect_Measure_DataWith_ExclusionCount = 0;
                     IF(@blnMeasureDataExists > 0)
                         BEGIN
				--print('spValidateImportExam validate-9-1 inside'+CONVERT( VARCHAR(24), GETDATE(), 113));
                             EXEC dbo.spValidateImportExamMeasureData
                                  @Import_examID,
                                  @iParentNode;
								--print('spValidateImportExam validate-9-2 inside'+CONVERT( VARCHAR(24), GETDATE(), 113));
				...
				--select @intInCorrectMeasureCount = COUNT(*) from tbl_Import_Exam_Measure_Data where Import_ExamID = @Import_examID and (Error_Codes_Desc is not null)
                             SELECT @intCorrectMeasureCount = CASE
                                                                  WHEN((Error_Codes_Desc IS NULL)
                                                                       AND (Warning_Codes_Desc IS NULL)
                                                                       AND (Exclusion_Codes_Desc IS NULL) -- Change #5:
                                                                       AND (([Status] = 3)
                                                                            OR ([Status] = 4)))
                                                                  THEN(@intCorrectMeasureCount + 1)
                                    ...
                                    @intCorrect_Measure_DataWith_WarningCount = CASE  -- Change #5:
                                                                                    WHEN((Warning_Codes_Desc IS NOT NULL)
                                                                                         AND (([Status] = 3)
                                                                                              OR ([Status] = 4)))
                                                                                    THEN(@intCorrect_Measure_DataWith_WarningCount + 1)
                                                                                    ELSE @intCorrect_Measure_DataWith_WarningCount
                                                                                END,
                                    @intInCorrect_Measure_DataWith_ExclusionCount = CASE
                                                                                        WHEN(((Exclusion_Codes_Desc IS NOT NULL)
                                                                                              AND (Error_Codes_Desc IS NULL))
                                                                                             AND (([Status] <> 3)
                                                                                                  OR ([Status] <> 4)))
                                                                                        THEN(@intInCorrect_Measure_DataWith_ExclusionCount + 1)
                                                                                        ELSE @intInCorrect_Measure_DataWith_ExclusionCount
                                                                                    END,
                                    @intInCorrectMeasureCount = CASE
                                                                    WHEN(((Error_Codes_Desc IS NOT NULL))
                                                                         AND (([Status] <> 3)
                                                                              OR ([Status] <> 4)))
                                                                    THEN(@intInCorrectMeasureCount + 1)
                                                                    ELSE @intInCorrectMeasureCount
                                                                END,
                                    @intSuccessMeasureCount = CASE [Status]
                                                                  WHEN 3
                                                                  THEN(@intSuccessMeasureCount + 1)
                                                                  ELSE @intSuccessMeasureCount
                                                              END,
                                    @intPartialSuccessMeasureCount = CASE [Status]
                                                                         WHEN 4
                                                                         THEN(@intPartialSuccessMeasureCount + 1)
                                                                         ELSE @intPartialSuccessMeasureCount
                                                                     END,
                                    @intValidationFailedMeasureCount = CASE [Status]
                                                                           WHEN 5
                                                                           THEN(@intValidationFailedMeasureCount + 1)
    ...
    --print('spValidateImportExam validate-10 inside'+CONVERT( VARCHAR(24), GETDATE(), 113));
    ...
    --set @message = 'Errors In Import exam : ' + @iParentNode + ']'+ CHAR(10) +  @message;
                             ...
                             UPDATE tbl_Import_Exam
                               SET
                                   ...
                                   InCorrect_Measure_DataWith_ExclusionCount = @intInCorrect_Measure_DataWith_ExclusionCount
                             WHERE Import_examID = @Import_examID;
                             PRINT '-------- Exam Report --------';
                             PRINT @message;
                             PRINT '-------- End Report --------';
	    --print('spValidateImportExam validate-11 inside'+CONVERT( VARCHAR(24), GETDATE(), 113));	 
                         END;
                         ELSE
                         BEGIN
    ...
    -- else @intCorrectMeasureCount > 0 then 3 successful?
                             UPDATE tbl_Import_Exam
                               SET
                                   ...
                                   Incorrect_Measure_DataCount = @intInCorrectMeasureCount,
                                   [Status] = CASE
                                                  WHEN((@intCorrectMeasureCount = 0)
                                                       AND (@intCorrect_Measure_DataWith_WarningCount = 0))
                                                      OR ((@intCorrectMeasureCount + @intCorrect_Measure_DataWith_WarningCount + @intInCorrectMeasureCount + @intInCorrect_Measure_DataWith_ExclusionCount - @intValidationFailedMeasureCount) < 1)
                                                  THEN 5
                                                  WHEN(((@intCorrectMeasureCount > 0)
                                                        OR (@intCorrect_Measure_DataWith_WarningCount > 0))
                                                       AND ((@intInCorrectMeasureCount > 0)
                                                            OR (@intInCorrect_Measure_DataWith_ExclusionCount > 0))
     ...
     --print('spValidateImportExam validate-12 inside'+CONVERT( VARCHAR(24), GETDATE(), 113));	 
